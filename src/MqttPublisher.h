...
public:
  void setup(MqttConfig _config) {
    ...
    baseTopic =
        String(config.topic) +
        (lastCharOfTopic >= 0 && config.topic[lastCharOfTopic] == '/' ? ""
                                                                      : "/");

    client.setServer(const_cast<const char *>(config.server),
                     atoi(config.port));
    ...
    client.setWill(String(baseTopic + MQTT_LWT_TOPIC).c_str(), MQTT_LWT_QOS,
                   MQTT_LWT_RETAIN, MQTT_LWT_PAYLOAD_OFFLINE);
    client.setKeepAlive(MQTT_RECONNECT_DELAY * 3);

    this->registerHandlers();
  }

  void debug(const char *message) { publish(baseTopic + "debug", message); }

  void info(const char *message) { publish(baseTopic + "info", message); }
...
private:
  ...
  void publish(const String &topic, const String &payload, uint8_t qos = 0,
               bool retain = false) {
    publish(topic.c_str(), payload.c_str(), qos, retain);
  }
  void publish(String &topic, const char *payload, uint8_t qos = 0,
               bool retain = false) {
    publish(topic.c_str(), payload, qos, retain);
  }
  void publish(const char *topic, const String &payload, uint8_t qos = 0,
               bool retain = false) {
    publish(topic, payload.c_str(), qos, retain);
  }

  void publish(const char *topic, const char *payload, uint8_t qos = 0,
               bool retain = false) {
    if (this->connected) {
  ...
  void registerHandlers() {
    client.onConnect([this](bool sessionPresent) {
      ...
      char message[64];
      snprintf(message, 64, "Hello from %08X, running SMLReader version %s.",
               ESP.getChipId(), VERSION);
      info(message);
      publish(baseTopic + MQTT_LWT_TOPIC, MQTT_LWT_PAYLOAD_ONLINE, MQTT_LWT_QOS,
              MQTT_LWT_RETAIN);
    });
    client.onDisconnect([this](int8_t reason) {
      this->connected = false;
      DEBUG("MQTT client disconnected with reason=%d", reason);
      reconnectTimer.attach(MQTT_RECONNECT_DELAY,
                            [this]() { this->connect(); });
    });
    client.onError([this](uint8_t e, uint32_t info) {
      switch (e) {
      case TCP_DISCONNECTED:
        // usually because your structure is wrong and you called a function
        // before onMqttConnect
      ...
      case UNRECOVERABLE_CONNECT_FAIL:
        // there is something wrong with your connection parameters? IP:port
        // incorrect? user credentials typo'd?
      ...
      case INBOUND_PUB_TOO_BIG:
        // someone sent you a p[acket that this MCU does not have enough FLASH
        // to handle
        DEBUG(F("MQTT: INBOUND_PUB_TOO_BIG size=%d Max=%d"), info,
              client.getMaxPayloadSize());
        break;
      case OUTBOUND_PUB_TOO_BIG:
        // you tried to send a packet that this MCU does not have enough FLASH
        // to handle
        DEBUG(F("MQTT: OUTBOUND_PUB_TOO_BIG size=%d Max=%d"), info,
              client.getMaxPayloadSize());
        break;
      case BOGUS_PACKET: //  Your server sent a control packet type unknown to
                         //  MQTT 3.1.1 99.99% unlikely to ever happen, but this
                         //  message is better than a crash, non?
        DEBUG(F("MQTT: BOGUS_PACKET info=%02x"), info);
        break;
      case X_INVALID_LENGTH: //  An x function rcvd a msg with an unexpected
                             //  length: probale data corruption or malicious
                             //  msg 99.99% unlikely to ever happen, but this
                             //  message is better than a crash, non?
