#ifndef SML_SENSOR_HPP_
#define SML_SENSOR_HPP_
...
public:
  ...
  SML_Sensor(const SensorConfig *config,
    ...
    if (this->config->status_led_enabled) {
      this->status_led =
          unique_ptr<JLed>(new JLed(this->config->status_led_pin));
...
private:
  ...
  void run_current_state() {
    if (this->state != INIT) {
      if (this->state != STANDBY &&
          ((millis() - this->last_state_reset) > (READ_TIMEOUT * 1000))) {
        DEBUG(
            "Did not receive an SML message within %d seconds, starting over.",
            READ_TIMEOUT);
        this->reset_state();
      }
      switch (this->state) {
  ...
  int data_available() { return this->serial->available(); }
  int data_read() { return this->serial->read(); }
  ...
  void set_state(State new_state) {
    if (new_state == STANDBY) {
      ...
      DEBUG("State of sensor %s is 'WAIT_FOR_START_SEQUENCE'.",
            this->config->name);
  ...
  void init_state() { this->set_state(WAIT_FOR_START_SEQUENCE); }
  ...
  void wait_for_start_sequence() {
    while (this->data_available()) {
      ...
      this->position =
          (this->buffer[this->position] == START_SEQUENCE[this->position])
              ? (this->position + 1)
              : 0;
  ...
  void read_message() {
    while (this->data_available()) {
      // Check whether the buffer is still big enough to hold the number of fill
      // bytes (1 byte) and the checksum (2 bytes)
      ...
      for (int i = 0; i <= last_index_of_end_seq; i++) {
        if (END_SEQUENCE[last_index_of_end_seq - i] !=
            this->buffer[this->position - (i + 1)]) {
          break;
        }
